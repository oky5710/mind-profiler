...
All six findings above were fixed on top of `1a92a22`. Release/test-gap unit tests were not added — flagging that gap remains open.

- **[High] Local cache fatalError**: `Services/RMSSDLocalStore.swift` init no longer calls `fatalError`. It now tries the on-disk store, and on failure deletes `RMSSDLocalCache` and retries once (the store is disposable/recomputable, so deleting beats trying to repair it); if that also fails it falls back to an in-memory `ModelContainer` (schema-only, can't fail) via a new `isUsingInMemoryFallback` flag, so a corrupt cache degrades to "recompute every time this session" instead of crash-looping.
- **[Medium] Merged sleep gap not counted as awake**: added `SleepAnalysisService.awakeIntervals(within:timeline:)`, which unions explicit `.awake` samples with any stretch inside the range that has zero timeline coverage at all (via a new `subtractIntervals` helper), matching the doc's stated behavior. Both `HomeViewModel.briefingNight` and `HRVAnalysisView.makeContinuityMetrics` now call this one helper instead of each inlining the same (buggy) logic.
- **[Medium] Overlapping sleep-stage samples double-counted**: added `SleepAnalysisService.nonOverlappingStageDurations`, which merges each stage's own samples, then subtracts whatever a higher-priority stage (`stageOverlapPriority`: deep > rem > core > unspecified — a separate constant from the display-order one, intentionally, so changing display order can't silently change overlap semantics) already claimed, before summing. `flushGroup` now calls this instead of a raw per-sample sum.
- **[Medium] Hard-coded cat-photo category list**: `CatPhotoService` no longer lists category folders. It resolves the bundled `CatPhotos` directory once and walks it with `FileManager.enumerator`, filtering `.jpg` files, so new/renamed/removed category folders are picked up automatically.
- **[Low] Splash decode on the calling actor**: `CatPhotoService.randomPhoto()` is now `async` and runs its bundle enumeration + `CGImageSourceCreateImageAtIndex` inside `Task.detached`, off whatever actor called it. `MindProfilerApp`'s splash `.task` now does `await CatPhotoService.randomPhoto()`.
- **[Low] Non-cancellation-aware operation queue**: `RMSSDLocalStore.acquireOperation()` is now `async throws` and wraps its queued wait in `withTaskCancellationHandler`. A cancelled waiter is spliced out of `operationWaiters` and resumed with `CancellationError` immediately (via a MainActor-hop from `onCancel`) instead of waiting for its natural turn and then running its now-obsolete request; a second `Task.isCancelled` check right after waking covers the race where cancellation lands just as the waiter is dequeued normally.

Build verified after each step (`xcodebuild ... build`, no errors). No runtime/simulator verification was done for the splash-photo async change or the sleep-gap arithmetic — worth exercising manually (a night with a real >0 but no-sample gap, and the splash on a cold launch) before treating this as fully verified.
